...
>>>>>>> 5b4de94 (feat(admin): enhance user activity details with inspector modal, 24h session telemetry, accurate WAT timestamps, and broadcast transcript separation)
...
<body class="min-h-screen flex flex-col antialiased">
      ...
      <main class="p-6 space-y-5 flex-1 max-w-7xl">
        ...
        <section id="view-chats" class="hidden space-y-3">
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 h-[680px]">
            ...
            <!-- RIGHT TRANSCRIPT STREAM (AUTHENTIC WHATSAPP THEME) -->
        ...
        <section id="view-broadcast" class="hidden space-y-4">
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <!-- RIGHT PREVIEW (AUTHENTIC WHATSAPP PREVIEW) -->
  ...
  <script>
    ...
    function filterStudentsTable() {
      ...
      const walletFilter = document.getElementById("wallet-filter") ? document.getElementById("wallet-filter").value : "ALL";
      
      const filtered = rawStudentsList.filter(s => {
        if (activeLevelFilter !== "ALL" && s.level !== activeLevelFilter) return false;
        
        if (walletFilter === "FUNDED" && (s.wallet_balance_ngn || 0) <= 0) return false;
        if (walletFilter === "ZERO" && (s.wallet_balance_ngn || 0) > 0) return false;
